...
void TButton_Add(Button *btn, char *name, int x, int y, int width, int height, float scale)
{
    ...
    if (btn->visible){
        ...
        if (btn->hover) glColor3f(0.1,0.1,0.6);//Назначение новых характеристик ргб
        else glColor3f(0.2,0.3,0.6);
...
void drawMap(Hero *obj)
{
    ...
    for (int i=0; i<H; i++) //чтение массива по вертикали
    {
        for (int j=0; j<W; j++) // чтение массива по горизонтали
        {
            if (TileMap[i][j]=='A')
            {
                renderAreaTexture(&tex_Blocks,j*tsize,i*tsize,81,40,40,40,0,0,true);
            }
            if (TileMap[i][j]=='B')
            {
                renderAreaTexture(&tex_Blocks,j*tsize,i*tsize,78,40,40,40,1,0,true);
            }
                   ...
                   int nCmdShow)
{
    ...
    createTexture(&tex_Blocks, "SpritePlace.png");
    ...
    while (!bQuit)
    {
        ...
        else
        {
            ...
            case 0:
               ...
               TButton_Add(&btn[1], "Quit", 270, 10, 250, 50, 3);
